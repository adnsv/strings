...
concept trimming_predicate = requires (F&& f, unsigned c) {
    { static_cast<F&&>(f)(c) } -> std::same_as<bool>;
};
...
[[nodiscard]] constexpr auto trim(
    convertible_to_string_view_input auto const& s, trimming_predicate auto tr, trim_side side = trim_side::front_and_back)
{
    using string_view_type = string_view_type_of<decltype(s)>;
    auto sv = string_view_type(s);
    using ct = string_view_type::value_type;
